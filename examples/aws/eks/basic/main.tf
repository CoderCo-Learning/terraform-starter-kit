...
module "eks" {
  ...
  max_size           = 3                        # Defaults to 3
  min_size           = 1                        # Defaults to 1
  instance_disk_size = 20                       # Defaults to 20
  instance_types     = ["t3.medium"]            # Defaults to ["t3.medium"]
  capacity_type      = "ON_DEMAND"              # Defaults to "ON_DEMAND"
  ami_type           = "AL2023_x86_64_STANDARD" # Optional - EKS will choose based on cluster version if not specified

  endpoint_private_access = false # Defaults to false
  endpoint_public_access  = true  # Defaults to true

  authentication_mode                         = "API" # Defaults to "API"
  bootstrap_cluster_creator_admin_permissions = true  # Defaults to true
